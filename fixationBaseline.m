function fixationBaseline(initSetting, experimentPars, cfsPars)
%function fixationBaseline(initSetting, experimentPars, cfsPars, idiomList, experimentFont)
	% Inherited from idiomFunctionalROI.m
	...
	resp.functional{resp.iBlock - 1}.bStampFix = GetSecs; % Recording block time stamp
		...
		while true,
			...
			if experimentPars.mri.s2.BytesAvailable ~= 0, % if BytesAvailable will check every loop? %here
				...
				if keyPress == experimentPars.mri.triggerCode, %remember to set up ignore 53 in get response
					tmp = GetSecs;
					resp.functional{resp.iBlock - 1}.tbStampFix = tmp; % Recording trigger time stamp (block-wise)

					%resp.functional{resp.iBlock - 1}.tStampFix(g.tCount) = tmp; % Recording trigger time stamp (trigger-wise)
					%g.tCount = g.tCount + 1;
					break;
				end;
			end;
	...
	resp.functional{resp.iBlock - 1}.blockSotFix = initSetting.sot; % Recording block time stamp
	%resp.functional{resp.iBlock - 1}.trialSotFix(i) = initSetting.sot; % Recording trial time stamp
	% Prevent the flick between fixation transition
	drawFixPointCFS(initSetting, experimentPars, cfsPars);
	drawHolderFrameCfs(initSetting, experimentPars, cfsPars);

	experimentPars.duration = experimentPars.fixationBaselineDur;
	Screen('DrawingFinished', initSetting.windowPtr);
