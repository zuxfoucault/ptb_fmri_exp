function interData = statIntermediatePretest(resp, experimentPars)

regularIdiomAC = extractIdiomAccurateRatePretest(resp, experimentPars);
...
for j = 1:nRegularSession,
	...
	idiomResp(nFunctionalROI + j) = resp.main{1,j}.idiom.keyCode(1);
	charResp(nFunctionalROI + j) = resp.main{1,j}.character.keyCode(1);
	...
	switch fix((resp.main{1,j}.blockSeq(1)-1)/2),
		case 0,
			...
			if resp.main{1,j}.character.keyCode == 79, % Not aware any characters
				visibilityIx(1, j) = 1;
			else,
				visibilityIx(1, j) = 0;
			end
...
summaryKeyRegRaw = [blockSeq(nFunctionalROI + 1 : nFunctionalROI + nRegularSession); idiomResp(nFunctionalROI + 1 : nFunctionalROI + nRegularSession); regularIdiomAC; charResp(nFunctionalROI + 1 : nFunctionalROI + nRegularSession)];

% After delete data
summaryKeyReg = [blockSeq(nFunctionalROI + find(visibilityIx == 1)); idiomResp(nFunctionalROI + find(visibilityIx == 1)); regularIdiomAC(find(visibilityIx == 1)); charResp(nFunctionalROI + find(visibilityIx == 1))];
...
CFS_idiom = ...
(sum(summaryKeyReg(2,:) == 80 & summaryKeyReg(1,:) == 1 & summaryKeyReg(3,:) == 1) + ...
sum(summaryKeyReg(2,:) == 79 & summaryKeyReg(1,:) == 1 & summaryKeyReg(3,:) == 0))/sum(summaryKeyReg(1,:) == 1)

CFS_random = ...
(sum(summaryKeyReg(2,:) == 80 & summaryKeyReg(1,:) == 2 & summaryKeyReg(3,:) == 1) + ...
sum(summaryKeyReg(2,:) == 79 & summaryKeyReg(1,:) == 2 & summaryKeyReg(3,:) == 0))/sum(summaryKeyReg(1,:) == 2)

Superimpose_idiom = ...
(sum(summaryKeyReg(2,:) == 80 & summaryKeyReg(1,:) == 3 & summaryKeyReg(3,:) == 1) + ...
sum(summaryKeyReg(2,:) == 79 & summaryKeyReg(1,:) == 3 & summaryKeyReg(3,:) == 0))*4/nRegularSession

Superimpose_random = ...
(sum(summaryKeyReg(2,:) == 80 & summaryKeyReg(1,:) == 4 & summaryKeyReg(3,:) == 1) + ...
sum(summaryKeyReg(2,:) == 79 & summaryKeyReg(1,:) == 4 & summaryKeyReg(3,:) == 0))*4/nRegularSession


CFS_idiomChar = sum(summaryKeyRegRaw(4,:) == 80 & summaryKeyRegRaw(1,:) == 1)*4/nRegularSession
CFS_randomChar = sum(summaryKeyRegRaw(4,:) == 80 & summaryKeyRegRaw(1,:) == 2)*4/nRegularSession
Superimpose_idiomChar = sum(summaryKeyRegRaw(4,:) == 80 & summaryKeyRegRaw(1,:) == 3)*4/nRegularSession
Superimpose_randomChar = sum(summaryKeyRegRaw(4,:) == 80 & summaryKeyRegRaw(1,:) == 4)*4/nRegularSession


interData = [CFS_idiom CFS_random Superimpose_idiom Superimpose_random CFS_idiomChar CFS_randomChar Superimpose_idiomChar Superimpose_randomChar];
CFS_averageAC = sum(interData(1:2))/2
Superimpose_averageAC = sum(interData(3:4))/2
